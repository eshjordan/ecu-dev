...
CRC calc_esp_checksum(ESP32Msg_t *const msg) { return msg->checksum = calc_crc(msg, sizeof(ESP32Msg_t) - sizeof(CRC)); }

int check_esp(const ESP32Msg_t *const msg)
{
    if (msg->header.start_byte != ECU_MSG_START_BYTE) { return -ERR_INVALID_START_BYTE; }

    if (msg->checksum != calc_crc(msg, sizeof(ESP32Msg_t) - sizeof(CRC))) { return -ERR_CRC_FAILED; }
    return 1;
}
...
/* 64-bit */
    ...
    sprintf(str, fmt_str, msg->adc[0], msg->adc[1], msg->adc[2], msg->adc[3], msg->adc[4], msg->adc[5], msg->adc[6],
            msg->adc[7], msg->adc[8], msg->hall_effect, msg->din1, msg->din2, msg->din3, msg->CANMsg.flags,
