...
static uint32_t ulTxRxCycles[echoNUM_ECHO_CLIENTS] = {0}, ulTxRxFailures[echoNUM_ECHO_CLIENTS] = {0},
                ulConnections[echoNUM_ECHO_CLIENTS] = {0};

/* Rx and Tx buffers for each created task. */
static char cTxBuffers[echoNUM_ECHO_CLIENTS][echoBUFFER_SIZES], cRxBuffers[echoNUM_ECHO_CLIENTS][echoBUFFER_SIZES];
...
static void prvEchoClientTask(void *pvParameters)
{
    ...
    xEchoServerAddress.sin_addr = FreeRTOS_inet_addr_quick(configECHO_SERVER_ADDR0, configECHO_SERVER_ADDR1,
    ...
    for (;;)
    {
        ...
        if (ret == 0)
        {
            ...
            for (lLoopCount = 0; lLoopCount < lMaxLoopCount; lLoopCount++)
            {
                ...
                {
                    /* Error? */
                    printf("Error sending data to the echo server \n");
                    break;
                }
                ...
                while (xReceivedBytes < lTransmitted)
                {
                    ...
                    if (xReturned < 0)
                    {
                        ...
                        below. */
                        printf("Error receiving data from the echo server \n");
                        xReceivedBytes = xReturned;
                        break;
                    } else if (xReturned == 0)
                    {
                        /* Timed out. */
                        printf("Timed out receiving data from the echo server \n");
                        break;
                    } else
                    {
                ...
                if (xReceivedBytes > 0)
                {
                    ...
                    if (strncmp(pcReceivedString, pcTransmittedString, lTransmitted) == 0)
                    {
                        ...
                        printf("RECEIVED GOOD MSG\n");
                        ulTxRxCycles[xInstance]++;
                    } else
                    {
                        /* The received string did not match the transmitted
                        string. */
                        printf("RECEIVED BAD MSG\n");
            ...
            {
                xReturned =
                    FreeRTOS_recv(xSocket,                /* The socket being received from. */
...
int main(void)
{
    /* Initialise the network interface.  The network interface should already
    be initialised by the power on reset code. */
    FreeRTOS_IPInit(ucIPAddress, ucNetMask, ucGatewayAddress, ucDNSServerAddress, ucMACAddress);

    vStartTCPEchoClientTasks_SingleTasks(1000, tskIDLE_PRIORITY);
